...
router.post("/change-user-account-description/", body('ChanelDescription').isLength({ min: 10, max: 100 }).not().isEmpty().trim(), body('AccountToken').not().isEmpty().trim().escape(), UserAccountService.UpdateChanelDescription);
